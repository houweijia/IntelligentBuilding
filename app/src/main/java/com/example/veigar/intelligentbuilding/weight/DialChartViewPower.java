...
public class DialChartViewPower extends GraphicalView {
	...
	public DialChartViewPower(Context context) {
		super(context);
		initView();
	}

	public DialChartViewPower(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	}

	public DialChartViewPower(Context context, AttributeSet attrs, int defStyle) {
	...
	public void addAxis() {
		...
		for (int i = 0; i <= 200;) {
			if (0 == i || j == 2) {
				rlabels.add(Integer.toString(i)+"W");
				j = 0;
			} else {
				rlabels.add("");
				j++;
